...
import { miscellaneousSchema } from "~/schema/miscellaneous.schema";
...
const { showModal } = storeToRefs(miscellaneousModalStore);
...
const validationSchema = toTypedSchema(miscellaneousSchema);
...
const [area] = defineField("area"); // Define area field
...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    ...
    const response = await $apiFetch("/api/miscellaneous-tasks/", {
      method: "POST",
      body: {
    ...
    toast.add({
      title: "Success",
      description: "Miscellaneous task has been created successfully.",
      color: "success",
    ...
    resetForm();
    miscellaneousModalStore.close();
    emit("clearAddDeadlineForm");
  } catch (error) {
    console.error("Error creating Miscellaneous task:", error);
    toast.add({
...
<template>
  <UModal
    ...
    title="Miscellaneous Task Details - Step 2 of 2"
        ...
        <div class="bg-primary-50 p-6 rounded-lg">
          ...
          <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <UFormField
              ...
              :error="errors.area"
              required
              help="Specify the area related to the task"
            >
              <UInput
                v-model="area"
