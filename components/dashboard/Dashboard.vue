...
const { enhancedStats, performanceMetrics, isAnyLoading, selectedDateRange } = storeToRefs(dashboardStore)
...
// Filter handlers
const handleStartDateUpdate = async (startDate: string | null): Promise<void> => {
...
const getPriorityData = () => {
  ...
  const getOnTimeCompletionRate = () => {
    ...
    console.log('getOnTimeCompletionRate - performanceMetrics:', performanceMetrics?.value)
    console.log('getOnTimeCompletionRate - enhancedStats:', enhancedStats?.value)
    console.log('getOnTimeCompletionRate - enhancedStats perf metrics:', enhancedStats?.value?.performance_metrics)
    ...
    const fallbackRate = enhancedStats?.value?.performance_metrics?.on_time_completion_rate
    ...
    if (summary) {
      ...
      console.log('getOnTimeCompletionRate - total:', total, 'completed:', completed, 'overdue:', overdue)
      ...
      if (total > 0 && completed > 0) {
        ...
        console.log('getOnTimeCompletionRate - calculated onTimeRate:', onTimeRate)
      ...
      if (statusData) {
        ...
        const totalTasks = Object.values(statusData).reduce((sum: number, count: any) => sum + (count as number), 0) as number

        if (totalTasks > 0) {
          const onTimeRate = (completedCount / totalTasks) * 100
          console.log('getOnTimeCompletionRate - using status breakdown:', onTimeRate)
          return Math.round(Math.max(0, Math.min(100, onTimeRate)))
        }
      }
    }
    ...
    if (teamAnalytics?.user_performance?.length > 0) {
      ...
      const onTimeUsers = teamAnalytics.user_performance.filter((user: any) =>
        user.completion_rate && user.completion_rate > 80 // Assume >80% is on-time
      ).length
  ...
  const getAverageCompletionDays = () => {
    // Debug: Log the data we're working with
    console.log('getAverageCompletionDays - performanceMetrics:', performanceMetrics?.value)
    console.log('getAverageCompletionDays - enhancedStats:', enhancedStats?.value)
    ...
    // Fallback: try enhancedStats path
    const fallbackDays = enhancedStats?.value?.performance_metrics?.average_completion_days
    if (fallbackDays !== undefined && fallbackDays !== null && fallbackDays > 0) {
      console.log('getAverageCompletionDays - using fallback days:', fallbackDays)
    ...
    if (summary) {
      ...
      console.log('getAverageCompletionDays - completed:', completed, 'total:', total, 'overdue:', overdue)
      ...
      if (completed > 0) {
        ...
        console.log('getAverageCompletionDays - completionRate:', completionRate, 'estimatedDays:', estimatedDays)
        return Math.max(1, Math.min(30, estimatedDays)) // Clamp between 1-30 days
      }
    }

    // Try business intelligence data
    const systemHealth = enhancedStats.value?.business_intelligence?.system_health
    if (systemHealth?.average_tasks_per_client) {
      ...
      console.log('getAverageCompletionDays - using system health:', estimatedDays)
...
const getSystemHealthTrendData = () => {
  ...
  if (last7Weeks.length > 0) {
    const data = last7Weeks.map((trend: any) => {
      ...
      return trend.active_clients || trend.total_active_clients || trend.active_client_count ||
             trend.system_active_clients || trend.total_clients || trend.client_count || 0
    })
  ...
  if (systemHealth) {
    ...
    if (activeClients > 0 || totalClients > 0) {
      ...
      return [
        ...
        Math.round(baseValue * 0.90), // Day 5
      ...
      <div v-if="isAnyLoading" class="loading-overlay">
        ...
        <div v-else class="dashboard-main p-4 md:p-6 space-y-4 md:space-y-6">
         <!-- Key Metrics Row -->
          <section class="key-metrics">
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 md:mb-6 gap-4">
              <h2 class="text-lg md:text-xl font-semibold text-gray-900 dark:text-white">Key Metrics</h2>
        ...
        <!-- Performance Overview -->
         <section class="performance-overview">
           <h2 class="text-lg md:text-xl font-semibold text-gray-900 dark:text-white mb-4 md:mb-6">Performance Overview</h2>
           <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
              <TrendCard
                title="Completion Rate"
                :value="dashboardStore.performanceMetrics?.overall_completion_rate || 0"
                format="percentage"
                value-label="Overall completion rate"
                :trend="{ direction: 'up', percentage: 5.2 }"
                :secondary-metrics="[
                   { label: 'On Time Rate', value: `${getOnTimeCompletionRate()}%` },
                   { label: 'Avg Days', value: `${getAverageCompletionDays()} days` }
                ]"
              />
               <TrendCard
        ...
        <!-- Charts Section -->
          <section v-if="isAdmin" class="charts-section">
            <h2 class="text-lg md:text-xl font-semibold text-gray-900 dark:text-white mb-4 md:mb-6">Analytics</h2>
          ...
          <section v-if="isAdmin" class="insights-section">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-4 md:mb-6">Team & Client Insights</h2>
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
              <!-- Team Performance -->
          ...
          <section v-if="isAdmin" class="business-intelligence">
            <h2 class="text-xl font-semibold text-gray-900 dark:text-white mb-4 md:mb-6">Business Intelligence</h2>
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 md:gap-6">
              <!-- Approval Workflow -->
